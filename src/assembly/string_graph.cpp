...
bool StrGraph::readAsqgFile(char* filename)
{
  ...
  else				// succeed to open
  {
    ...
    /***********/
    p_order = p_seq.size();
    std::cout<<"order : "<<p_seq.size()<<"\t"<<p_readID.size()<<"\n";
...
// perform DFS and compute local components
void StrGraph::computeLocalComponents() {
    //std::vector<bool> visited(boost::num_vertices(*p_graph_), false);
    std::set<int> currentComponent;
    std::vector<std::set<int>> components;
    BooleanVector visited = BooleanVector(2*p_order, false);

     for(auto it = boost::vertices(*p_graph_).first; it != boost::vertices(*p_graph_).second; ++it) {
        int rid = (*p_graph_)[*it].rid_;
        if (!visited[rid]) {
           std::stack<BoostSTRVertex> dfsStack;
            //std::stack<int> dfsStack;
            dfsStack.push(*it);

            while (!dfsStack.empty()) {
                const auto v = dfsStack.top();
                dfsStack.pop();
                int vid = (*p_graph_)[v].rid_;
                if (!visited[vid]) {
                    visited[vid] = true;
                    currentComponent.insert(vid);

                    const auto outEdges = boost::out_edges(v, *p_graph_);
                    for (auto edgeIt = outEdges.first; edgeIt != outEdges.second; ++edgeIt) {
                        const auto target = boost::target(*edgeIt, *p_graph_);
                        if (!visited[(*p_graph_)[target].rid_]) {
                            dfsStack.push(target);
                        }
                    }

                    const auto inEdges = boost::in_edges(v, *p_graph_);
                    for (auto edgeIt = inEdges.first; edgeIt != inEdges.second; ++edgeIt) {
                        const auto source = boost::source(*edgeIt, *p_graph_);
                        if (!visited[(*p_graph_)[source].rid_]) {
    ...
    std::string outname = "local_components.txt";
    std::ofstream fout(outname.c_str());

     // Print the result
    for (const auto& component : components) {
        if(component.size() > 100 && component.size() < 10000){
          fout << "\nComponent: ";
           for (const auto& vertex : component) {
            //int r_id = (*p_graph_)[vertex].rid_;
            int r_id = vertex;
  ...
  for(auto it = boost::vertices(*p_graph_).first; it != boost::vertices(*p_graph_).second; ++it)
  {
    if(! p_crossed[ (*p_graph_)[*it].rid_ ]) // untraversed vertex, new component
    {
      ...
      for(auto it_v = this_component.begin(); it_v != this_component.end(); ++it_v)
      {
        ...
        {
          //count++;
          //std::cout<<"Component no "<<count<<"\n";
          //std::cout<<"Seed :"<<(*p_graph_)[*it_v].rid_<<"\n";
...
void StrGraph::condense(const BoostSTRVertex seed, std::list<cycle_s>& to_add_cycle)
  {
    //std::cout<<"Edges in "<<(*p_graph_)[seed].rid_<<" component\n";
    for(auto it_e = boost::out_edges(seed, *p_graph_).first; it_e != boost::out_edges(seed, *p_graph_).second; ++it_e)
    {
      //BoostSTREdge head_edge = *it_e;
...
void StrGraph::condense(const BoostSTREdge source_edge, std::list<cycle_s>& to_add_cycle)
    {
...
void StrGraph::DFS(int vertex_id, std::stack<int>& vertex_stack) 
{
    std::vector<bool> visited(boost::num_vertices(*p_graph_), false);  // use vector of bools to track visited vertices
	vertex_stack.push(vertex_id);  // push current vertex to stack
    ...
    while (!vertex_stack.empty()) 
	{
        ...
        if (!visited[current_vertex]) 
		{
			...
			// traverse edges and access vertices using boost::target
			for(auto it_e = boost::edges(*p_graph_).first; it_e != boost::edges(*p_graph_).second; ++it_e)
			{
				t = boost::target(*it_e, *p_graph_);
				int v = (*p_graph_)[t].rid_; // retreiving id

				if (v == vertex_id) 
				{
					break; // found the vertex
				}
			}   
			...
			if (!t)
			{
				// if the vertex t is still null by this point, then an invalid vertex was given in the previous code block
				throw std::runtime_error("null vertex");
			}

			for (auto it_out_e = boost::out_edges(t, *p_graph_).first; it_out_e != boost::out_edges(t, *p_graph_).second; ++it_out_e) 
			{
				BoostSTRVertex adjacent_vertex = boost::target(*it_out_e, *p_graph_);
				int adjacent_vertex_id = (*p_graph_)[adjacent_vertex].rid_; // retreiving id
							
				if (!visited[adjacent_vertex_id]) 
				{
				vertex_stack.push(adjacent_vertex_id);
				}
            }
        }
    }
...
void StrGraph::writeGraph(char* filename, IntegerSet component) // integrate components into code
{
	/** timer **/
	clock_t t;
	t = clock();
